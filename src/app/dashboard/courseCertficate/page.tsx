...
import { Select, ConfigProvider, Modal, Image, Upload, message } from "antd";
...
const Page = () => {
  ...
  const handleManualPreview = () => {
    if (fileList.length > 0) {
      ...
      message.error("Nothing is Uploaded");
    }
  };
  ...
  const beforeUpload: UploadProps['beforeUpload'] = (file: RcFile): boolean | Promise<boolean> => {
    if (file.size < 2 * 1024 * 1024) {
      console.log("File size is valid (below 2 MB).");
      return true;
    } else {
      console.log("File size exceeds the limit of 2 MB.");
      message.error("File size exceeds the limit of 2 MB.");
      return false;
    }
  };
        ...
        <div className="w-full 2xl:w-[87%] flex relative">
          ...
          <div className="overflow-y-scroll w-[100%] h-[88vh] scroll-side">
            <div className="flex flex-col bg-transparent gap-[20px] p-[20px] relative">
              ...
              <div className="relative flex course-cer-main flex-col min-[986px]:flex-row gap-[30px] min-[926px]:gap-0 min-[926px]:items-start mt-[20px] md:mt-[0px] items-center justify-between 2xl:w-[80%]">
                <div className="w-[310px] h-[448px] border-0.4 border-dark border-dashed md:ml-[-22px] mt-[-10px] rounded-[5px] flex flex-col items-center justify-center gap-[25px] p-[10px]">
                  {["PPF", "POFF", "CMRF", "ISOFR", "KOMPRSK"].map((item, index) => (
                    <div className="flex justify-between items-center w-[90%]" key={index}>
                      <div className="flex items-center gap-[10px]">
                        <Images src={Badge} alt="" className="max-w-[26px] max-h-[26px]" />
                        <p className="text-[24px] font-[500]">{item}</p>
                      </div>
                      <p className={`text-[16px] font-[500] ${index % 2 === 0 ? "text-[#12B28C]" : "text-[#FF2700]"}`}>
                        {index % 2 === 0 ? "Uploaded" : "Missing"}
                      </p>
                    </div>
                  ))}
                </div>
                <div>
                  ...
                  <div className="flex items-center justify-center mt-[170px] gap-[20px] flex-col course-cer-file min-[1103px]:flex-row">
                      ...
                      <ImgCrop rotationSlider>
                        ...
                        >
                          {fileList.length >= 1 ? (
                            <div className="flex flex-col items-center justify-center">
                              <button disabled>
                                <Images src={Icom} alt="Upload Icon" className="w-[44px]" />
                              </button>
                              <button
                                disabled
                    ...
                    <span className="w-[3px] h-[35px] border-r border-dashed border-[#7E7E7ED1]" />
                    <div className="flex gap-[10px] items-start">
                      <Images src={UplFile} alt="" className="min-w-[20px] max-w-[20px]" />
                      <div>
                        <p className="text-[500] text-[14px] max-w-[200px]">
                          {fileList.length >= 1 ? fileList[0].name : "No File Selected"}
                        </p>
